...
if modulo == "Filtros" :
    ...
    st.write(df)     #equivalente a print
    ...
    ingreso_indice=st.number_input("Ingrese el valor del indice")

    df_filtro1=df.loc[int(ingreso_indice),seleccion_columnas]
    st.write(df_filtro1)

    indice_columna=st.number_input("Ingrese el indice de la columna")
    indice_fila=st.number_input("Ingrese el indice de la fila")

    df_filtro2=df.iloc[int(indice_fila),int(indice_columna)]
    st.write(df_filtro2)
elif modulo=="Consultas" :

    print("Modulo Consultas")
    consulta=st.text_input("Ingrese el query")      #ejm:salary_in_usd>2000
